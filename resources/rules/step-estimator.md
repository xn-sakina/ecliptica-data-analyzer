...
`prior_transitions` 次后到达约 `0.995`。这里的近似值只用于弱先验拟合，不作为
实际进入 Jim 的判定阈值。
...
从当前 phase 开始反复应用推进模型，直到 phase 到达 `1.0`，得到
...
终点必须使用游戏定义的精确 `1.0`，不能用近似阈值提前吸收尾部误差。真实日志已出现
`phase = 0.887341` 后进入 `phase = 0.9994387` 普通关卡、再进入 Jim 的路径；若把
`0.995` 当作终点，会在 `0.887341` 后错误地少算这一场普通战。
...
- 2026-09-10 完整序列在 `phase = 0.887341` 的大厅必须估计还剩 1 场普通战，
  并在观察到 `phase = 0.9994387` 普通关卡后变为 0；
- 同一 phase 轨迹在中途加入且每回合分别为 4、10、25 分钟时，不能被时间先验推成不同回合；
- 88 分钟和 165 分钟到达 Jim 准备大厅的完整路径都必须保留相同 Jim 边界；
- 慢速、低推进的 16 回合轨迹必须在完整加入与中途加入时都保留额外普通回合，
  恢复为 `15/2 → 16/1`，不能在预测 phase 尚未到 `1.0` 时提前归零；
