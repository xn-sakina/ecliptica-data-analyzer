    ...
    fn layout_rows(
        ...
        for (idx, item) in state.items.iter().enumerate() {
            ...
            if item.config.shrink {
                if let Some(shrunk_item_cross_size) = state.shrunk_item_cross_size {
                    current_row.cross_size =
                        f32::max(current_row.cross_size, shrunk_item_cross_size);
                }
            }
        }
        ...
        let res = self.row_ui.scope_builder(
            ...
            |ui| {
                ...
                let res = if let Some(row) = row {
                    ...
                    if do_shrink {
                        if let Some(shrunk_item_cross_size) = res.shrunk_item_cross_size {
